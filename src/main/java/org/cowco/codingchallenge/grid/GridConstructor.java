...
public class GridConstructor {
     ...
     * TODO Perhaps we want to generate *all* valid word squares, rather than simply
    ...
    public void constructGrid(int dimension, String letters) {
        ...
        if (letters.length() != dimension * dimension) {
            LOGGER.error("Wrong number of letters! There should be " + dimension * dimension
                    + " letters but there are actually " + letters.length());
        } else {
            gridDim = dimension;
            ...
            if (dict.numWords() > 0) {
                ...
                recursiveGridCreator(0, new ArrayList<>(), letters);
            }
    ...
    /**
     * We utilise the fact that the valid words for the square are determined by the
     * leading letters in the previous words
     * 
    ...
    /**
     * Recursively generates the grid, by finding permutations of words with the
     * correct prefixes, using the prefix map generated by generatePrefixMap
     * 
     * @param gridRow          Which row of the grid we are on - determines the
     *                         length of the prefix to use
     * @param runningGridWords The grid that we are currently building
     */
    private void recursiveGridCreator(int gridRow, List<String> runningGridWords, String allowedLetters) {
        // Recursion end-condition
        if (gridRow == gridDim) {
            if (isGridValid(allowedLetters, runningGridWords)) {
                gridContents = new ArrayList<>(runningGridWords);
            }
        } else if (runningGridWords.size() != gridDim) {
            ...
            for (String prefixedWord : prefixes.get(currentPrefix)) {
                ...
                recursiveGridCreator(gridRow + 1, runningGridWords, allowedLetters);
    ...
    /**
     * Our filtering ensures that no letters *outside* the list are used, but we
     * need to make sure that all the letters *in* the list are used, in the correct
     * numbers.
     * 
    ...
    private boolean isGridValid(String availableLetters, List<String> grid) {
        ...
        List<Character> lettersToUse = availableLetters.chars().mapToObj(c -> (char) c).toList();
        String fullGrid = grid.stream().collect(Collectors.joining(""));
        List<Character> gridCharacters = fullGrid.chars().mapToObj(c -> (char) c).toList();

        for (Character letter : lettersToUse) {
            long numInGrid = gridCharacters.stream().filter(c -> c.equals(letter)).count();
            long numAvailable = lettersToUse.stream().filter(c -> c.equals(letter)).count();

            if (numAvailable != numInGrid) {
                isValid = false;
                LOGGER.debug("There are " + numAvailable + " of letter " + letter + " available, but " + numInGrid
                        + " in this grid!");
                break;
            }
        }
